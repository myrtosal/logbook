 ...
 * @author Savvas
 */
public class view_detailed_post extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<div class=\"post_container_detailed\">");
            out.println("<div class=\"post_area_detailed\" id=\"posts_area\">");
            out.println("<div class=\"post border rounded\">");
            out.println("<div class=\"post_top\">");
            ...
            //out.println("<a href=\"javascript:delete_post('" + post.getPostID() + "');\">Delete post</a>");
            //out.println("</div>");
            out.println("<div>");
            out.println("<span id=\"createdat\">" + post.getCreatedAt() + "</span>");
            out.println("<span id=\"area_info_display\"></span>");
            out.println("<span id=\"lat\">" + filter(post.getLatitude()) + "</span>");
            out.println("<span id=\"lon\">" + filter(post.getLongitude()) + "</span>");
            out.println("</div>");
            ...
            if (!post.getImageBase64().equals("") && post.getImageBase64() != null && !post.getImageBase64().equals("null")) {
                ...
                out.println("<img src=\"dasdasa\" hidden onerror=\"draw_camera_photo_detailed('" + post.getImageBase64() + "','" + "canvas" + i + "');\">");
                out.println("<div id=\"canvas" + i + "\"></div>");
                out.println("</div>");
            } else if (!post.getImageURL().equals("") && post.getImageURL() != null && !post.getImageURL().equals("null")) {
                out.println("<div class=\"image_area\" id=\"image_area" + i + "\">");
                out.println("<img id=\"image" + i + "\" src=\"" + filter(post.getImageURL()) + "\">");
                out.println("</div>");
            }
            ...
            out.println("</div>");
            out.println("</div>");
            out.println("<div id=\"place_answer\"></div>");
            out.println("</div>");
            
            out.println("</div>");
            
            out.println("<div class=\"map_place\" id=\"map_area\">");

            out.println("</div>");
            i++;
        } catch (ClassNotFoundException ex) {
            Logger.getLogger(view_detailed_post.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
